...
void Mount_SD (const TCHAR* path)
{
	...
	if (fresult != FR_OK) Send_Uart ("CANNOT MOUNT SD CARD\n");
...
void Unmount_SD (const TCHAR* path)
{
	...
	else Send_Uart("CANNOT UNMOUNT SD CARD\n");
}
...
FRESULT Create_File (char *name)
{
	...
	{
		char *buf = pvPortMalloc(100*sizeof(char));
		sprintf (buf, "ERROR!!! *%s* ALREADY EXISTS!! USE Update_File()\n", name);
		Send_Uart(buf);
		vPortFree(buf);
	    return fresult;
	}
	else
	{
		...
		{
			char *buf = pvPortMalloc(100*sizeof(char));
			sprintf (buf, "*%s* CREATED SUCCESSFULLY. NOW USE Write_File() TO WRITE DATA\n",name);
			Send_Uart(buf);
			vPortFree(buf);
		}
